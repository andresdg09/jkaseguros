  ...
  try {
    ...
    if (parseInt(polizasCheck.rows[0].count) === 0) {
      ...
      const resPol1 = await client.query(`
        ...
        VALUES ('POL-882731', $1, $2, $3, 'PLATINO', 5000, 0, 260, 'vigente', 'pagado') RETURNING id
      ...
      const resPol2 = await client.query(`
        INSERT INTO polizas (codigo_poliza, cliente_id, asesor_id, compania_id, plan, suma_asegurada, deducible, prima_anual, estado, pago_estado)
        VALUES ('POL-449201', $1, $2, $3, 'SALUD INDIVIDUAL', 30000, 0, 340, 'negociacion', 'pendiente') RETURNING id
      `, [clientDbIds[1], advisorDbIds[1], idMap['Seguros Caracas'] || 2]);
      ...
      const resPol3 = await client.query(`
        INSERT INTO polizas (codigo_poliza, cliente_id, asesor_id, compania_id, plan, suma_asegurada, deducible, prima_anual, estado, pago_estado)
        VALUES ('POL-102938', $1, $2, $3, 'BRONCE', 50000, 0, 450, 'vencido', 'pendiente') RETURNING id
      `, [clientDbIds[2], advisorDbIds[2], idMap['Seguros Venezuela'] || 3]);
